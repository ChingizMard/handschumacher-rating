...
.controller('RateCtrl', function($scope, $rootScope, $http, $ionicPopup) {
    ...
    //Geta all of the names from the Dashboard menu
    $scope.namesAndRates = new Array();
    console.log($rootScope.stations);
    for (station in $rootScope.stations) {
        $scope.namesAndRates.push({
            name: $rootScope.stations[station].name,
            rating: 0
        });
    }
    ...
    $scope.submitData = function() {
        ...
        //Submit rating for every stations
        for (entry in $scope.namesAndRates) {
            ...
            var uri = 'http://tux64-14.cs.drexel.edu:9000/rating/' + dayTime + '/' + $scope.namesAndRates[entry].name;
            if ($scope.namesAndRates[entry].rating != 0) { //Only submit if the value was changed
                $http({
                    method: 'POST',
                    url: uri,
                    headers: {
                    ...
                    data: {
                        rating: $scope.namesAndRates[entry].rating
                    }
...
.controller('DashCtrl', function($scope, $http, $rootScope, $timeout) {
    ...
    $scope.$on('$ionicView.enter', function() {
        ...
        $scope.averageRateFinal = 0; //Average rating for the Hans for DashBoard
        $scope.rate = Math.round($scope.averageRate);
        $scope.oneOpenOnly = true;
        $scope.stations = new Array();

        $scope.one = 0; //Average number of 'one' votes
        $scope.two = 0;
        $scope.three = 0;
        $scope.four = 0;
        $scope.five = 0;
        ...
        var populateRating = function(name, description) {
            ...
            if (currentHour > 0 && currentHour < 1200) {
                //Request the rating for the station
                ...
                $http({
                    ...
                    try { //Check if data exists
                        //Total number of votes for the station
                        ...
                        //Combining number of separate votes
                        ...
                        //Means no data yet. Set everything to zero
                        numberOfVotes = 0;
                        $scope.averageRate = 0;
                        $scope.one = 0;
                        $scope.two = 0;
                    ...
                    if (numberOfVotes != 0) {
                        ...
                        //Save everything to scope to pass it to a view.
                        $scope.stations.push({
                            name: name,
                        ...
                        //Push zeros if data was not created yet.
                        $scope.stations.push({
                            name: name,
                            description: description,
                            ratings: 0,
        ...
        //Calculate all of the final averages.
        $timeout(function() {
